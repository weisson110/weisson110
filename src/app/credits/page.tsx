    ...
    <div className="mx-auto max-w-[1000px] px-4 py-8 sm:px-6">
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-3">
        <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-5 md:col-span-1">
          ...
          <div className="mt-3 text-[32px] font-semibold text-white">${credits.balance.toFixed(2)}</div>
          <div className="mt-1 text-[12px] text-zinc-500">${credits.totalUsed.toFixed(2)} {t.keysPage.used} • ${credits.totalPurchased} purchased</div>
        ...
        <div className="md:col-span-2 space-y-4">
          <div className="rounded-xl border border-zinc-800 bg-zinc-900/30 p-5">
            ...
            <div className="mt-4 grid grid-cols-3 gap-4 text-[12px]">
              ...
              <div><div className="text-zinc-500">{t.creditsPage.totalCost}</div><div className="mt-1 text-[18px] font-semibold text-white">${credits.totalUsed.toFixed(2)}</div></div>
            </div>
            ...
            <div className="mt-4 space-y-2">
              {credits.transactions.map((tx: any) => (
                <div key={tx.id} className="flex items-center justify-between rounded-lg border border-zinc-800 bg-zinc-900/50 px-4 py-3">
                  ...
                  <div className={`text-[13px] font-medium ${tx.amountUsd > 0 ? "text-emerald-400" : "text-zinc-300"}`}>{tx.amountUsd > 0 ? "+" : ""}${tx.amountUsd.toFixed(2)}</div>
                </div>
              ))}
            </div>
          </div>
